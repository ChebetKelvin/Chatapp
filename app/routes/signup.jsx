...
import { validateEmail, validatePassword, validateText } from "../.server/auth";

export function meta() {
  return [
    { title: "Sign Up | ChatApp - Join the Conversation" },
    {
      name: "description",
      content:
        "Create your ChatApp account and start connecting with friends and colleagues instantly.",
    },
  ];
}
...
export async function action({ request }) {
  ...
  let fieldErrors = {
    ...
    terms: terms ? null : "You must agree to the terms",
    newsletter: null,
  };
  ...
  if (existingUser) {
    setErrorMessage(session, "Email already registered.");
    return redirect("/signup", {
      headers: { "Set-Cookie": await commitSession(session) },
    });
  }

  let createdUser = await addUser(user);

  if (createdUser.acknowledged) {
    setSuccessMessage(session, "Account created successfully! Please log in.");
    ...
    setErrorMessage(session, "Failed to create account. Please try again.");
    return redirect("/signup", {
      headers: {
        "Set-Cookie": await commitSession(session),
      },
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-8">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white mb-2">Join ChatApp</h1>
          ...
          <Form method="post" className="space-y-5">
            ...
            <div className="flex items-start space-x-3">
              <input
                ...
                id="terms"
                name="terms"
                className="w-4 h-4 text-blue-500 bg-white/5 border-white/20 rounded focus:ring-blue-500 focus:ring-2 mt-1"
                required
              />
              ...
              {fieldErrors.terms && (
                <p className="text-red-400 text-xs mt-1">{fieldErrors.terms}</p>
              )}
            </div>
        ...
        <div className="text-center mt-6">
          <p className="text-white/50 text-sm">
            🔒 End-to-end encryption • No credit card required
          </p>
        </div>
      </div>
    </div>
